use crate::ast::{Declaration, Expression, Span};
...
impl<'ctx> Compiler<'ctx> {
    /// Expression-level codegen: lower an `Expression` AST node to an LLVM
    /// `BasicValueEnum`. The largest codegen function (~1 600 lines) and the
    /// hottest dispatch path — covers literals, identifiers, calls, casts,
    /// deref, struct/enum instantiation, field/member/method access, if/block
    /// expressions, intrinsics, and match expressions. Phase 6 of the
    /// `compiler.rs` split (#113): moved verbatim from
    /// `src/codegen/compiler.rs` — behaviour-preserving code motion. The
    /// four `unsafe` blocks (in-bounds GEP for struct/enum field loads and
    /// array reads) move with the function; their scope is unchanged.
    pub(in crate::codegen) fn compile_expr(
        ...
        match e {
            ...
            Expression::Match {
                ...
                // Shared lowering with `Statement::Match` — see
                // `src/codegen/match_lowering.rs`. #183.
                let result = self.lower_match(cv, &ctn, arms, variables, function, false)?;
                Ok(result.unwrap_or(i64_t.const_zero().into()))
            }
