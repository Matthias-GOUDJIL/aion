...
impl<'a> Parser<'a> {
    ...
    /// Parse the `{ arm, arm, ... }` body of a `match` in statement or
    /// expression position. Shared by both call sites (#183); the two
            ...
            && self.current_token.kind != TokenKind::EOF
        {
            ...
            if is_valid_pattern {
                ...
                if patterns.len() == 1 {
                    ...
                    let followed_by_guard_or_arrow = self.current_token.kind == TokenKind::If
